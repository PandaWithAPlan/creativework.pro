...
### 2025-12-01 — Фаза 4, шаг 4.5: Микро-анимации и scroll-эффекты
- Использован существующий `scroll-animation.js` на базе IntersectionObserver, который следит за элементами с классом `.reveal-on-scroll` и при входе в видимую область добавляет модификатор `.is-visible` (с последующей отпиской от наблюдения). Это обеспечивает единый механизм появления блоков при прокрутке без дублирования логики.
- На главной странице (`src/index.html`) класс `.reveal-on-scroll` добавлен к ключевым секциям: `#hero`, `#pillars`, `#about-preview`, `#verticals`, `#benefits`, `#pricing`, `#case-studies`, `#blog-preview`, `#home-contact`. Таким образом, пользователь по мере скролла последовательно "открывает" историю хаба — от обещания ценности до тарифов, кейсов и подписки.
- На внутренних страницах `about.html`, `services.html`, `blog.html` и `faq.html` класс `.reveal-on-scroll` навешан на hero-секции и содержательные блоки (история, миссия, уровни сервисов, списки статей и FAQ-секции). Это создаёт единый паттерн мягкого появления контента при прокрутке без лишней анимации для футеров и сугубо вспомогательных элементов.
- В `src/css/animations.css` усилена поддержка доступности через `@media (prefers-reduced-motion: reduce)`: для пользователей, у которых в системе включено уменьшение анимации, элементы с `.reveal-on-scroll` отображаются сразу без смещения и переходов (отключены transform/transition/animation). Для остальных применяется плавный fade-in-up с небольшим смещением (12px) и длительностью ~0.4–0.5s.
